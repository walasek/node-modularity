...
class SystemState extends Module {
	...
	resolveClass(nameOrClassObject) {
		if (nameOrClassObject instanceof Object) {
			// Must check if we know it
			if (!Object.values(this.knownClasses).find(obj => obj.name === nameOrClassObject.name)) {
	...
	async setup() {
		...
		while (leftovers.length > 0) {
			...
			if (newLeftovers.length >= leftovers.length) {
				...
				throw Error(`Failed to initialize ${newLeftovers.length} modules. The error messages were: ${errmsg}`);
			}
